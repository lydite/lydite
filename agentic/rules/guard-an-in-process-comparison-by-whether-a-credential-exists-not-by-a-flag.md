...
conditional because the credential is. `clearance` is not that shape: the flow's `init-scm` stage
(`scmstages.InitSCM`) refuses the whole run with `ErrNoCredential` unless a `GITHUB_TOKEN` is set
(reading the head, the commenter's permission, the standing referral, and posting the reply all
need it), so every stage that can reach the comparison already holds a credential whether or not
this invocation also posts the status directly. Copying `review`'s conditional guard onto a
command like this leaves one branch — the one that "only renders" — running a component's own
build code beside a token it does hold, because the premise the condition tests for
(`--status-out`/`Render` implies no credential) is false for this command.
...
surfaces, err = reviewdecision.Surfaces(ctx, dir, baseSHA, document, statusOut == "", tc, progress)
