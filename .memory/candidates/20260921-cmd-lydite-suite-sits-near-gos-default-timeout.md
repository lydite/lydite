...
several runs on one machine, and one run tripped go test's default 10m limit: a timeout panic
with no `--- FAIL` line anywhere. The suite drives real end-to-end runs (mutation, fold,
record), so the time is the work rather than one slow test. (`grammar_subset_python` joined the
tag list after this candidate was first staged; the timing claim itself is unaffected — a fifth
grammar tag changes nothing about how long the suite's own end-to-end runs take.)
