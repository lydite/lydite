...
grammar_subset_tsx' ./cmd/lydite/...` measured 353s-588s wall-clock across several runs on one
machine, and one run tripped go test's default 10m limit: a timeout panic with no `--- FAIL`
line anywhere. The suite drives real end-to-end runs (mutation, fold, record), so the time is the
work rather than one slow test.
