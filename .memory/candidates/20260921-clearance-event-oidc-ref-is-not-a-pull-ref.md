...
- The relay accepts the clearance status only as `lydite/clearance`; `clearance.Context` in `source/cli/internal/clearance/decide.go` emits `lydite/referral`, so the two are out of step until the Go side moves.
