...
_Note_: a declaration covers the line the mutant is reported at and, for a statement spanning several lines, the line that statement closes on — where a trailing declaration is written. It covers neither the lines between, which belong to the sub-expressions written there, nor a neighbouring statement's. Reaching further would let a declaration already in the tree acknowledge code a later change adds beneath it, which is excluded from the run and never counted while the change adds no line carrying the token, so nothing refers it and it merges unread.
