...
**Aggregate coverage**:
Coverage as a ratio of covered to coverable lines, reported at two altitudes and gated at both: per **Component**, and over the repository. Both are the same quantity — `Σ covered / Σ total` — derived from one stored measurement per component, so they cannot drift apart or disagree. It catches regressions in code the current change never touches, which is the class **Patch coverage** cannot see at all.
_Note_: a language is not an altitude. It is a grouping derived from each component's runner rather than a unit anybody declared, so gating one holds a repository to a number it never stated — and where a language holds a single component the figure restates that component's own. See [ADR 0024](docs/adr/0024-coverage-gates-the-component-and-the-repository.md).
...
_Note_: the figures composed over the whole repository can be computed only where every component's counts are in one place. Under **Shard**s that is the fold and nowhere else, so a shard narrowed to part of the declaration reports its own components and leaves the repository-wide figures to the command that folds them.
...
_Note_: a **Component** whose suite was not already passing is unmeasured rather than zero-killed. Nothing can be concluded about tests that were failing before a single mutant ran, and a zero would read as a measured result rather than as a question that was never asked.
_Avoid_: "escaped mutant", "missed mutant" — other tools' vocabulary for the same thing. Also "uncaught", which reads as an exception.

**CRAP index**:
A per-function score, `comp² × (1 − cov)³ + comp`, pairing a function's cyclomatic complexity with the fraction of its statements a test executed. Coverage measures execution and nothing else, so a function nobody can follow scores full marks by being called once; the cubed term is what makes the pair a cliff rather than a slope — a complexity-12 function is 12 fully tested, 30 half tested and 156 untested. Scored in Go, Rust and TypeScript, each from a hand-rolled complexity walk rather than a pinned tool — see [ADR 0036](docs/adr/0036-crap-scores-rust-and-typescript-from-a-hand-rolled-walk.md).
_Note_: it is a **Gate**, cleared by testing the function or taking it apart. What is gated is the *delta* of how many functions sit above 30, per **Component** — never the absolute count, which would fail every repository on the day it upgrades over debt it has always had, and never scoped to the diff, which would miss a function pushed over the threshold by a change to its call site or by a deleted test.
_Note_: 30 is the threshold the definition names. It is not configurable and is not stored in the **Baseline**, so moving it is a change to what is measured rather than a knob — the reason the baseline's directory is keyed to its metric.
_Note_: the worst value is recorded beside the count and never gated. A change taking the worst function from 400 to 380 has improved nothing anybody can act on.

**Exclusion declaration**:
`[lydite:exclude_from_<gate>][<reason>]`, written as a line comment, saying one gate's finding is not evidence about this code. The gate is inside the token because the gates ask different questions of the same function: one whose coverage is taken in another process has not thereby become unmutable, and a score that is not evidence says nothing about whether the lines ran. `mutation` names an **Equivalent mutant**; `coverage` takes a function's lines out of both sides of every coverage figure; `crap` takes it out of the score alone.
_Note_: every one is a **Disqualifier**, so a change that adds one is referred and a human reads the claim. A reason is required, and a declaration that covers nothing is named rather than dropped.

**Equivalent mutant**:
A **Survivor** no test could ever kill, because the change it makes is unobservable — a deleted log line, an arithmetic rewrite with identical results over the reachable domain. Equivalence is undecidable in general, so lydite never tries to detect one: the author declares it, in a `[lydite:exclude_from_mutation][<reason>]` comment written on a line the mutant's replaced text spans, and the declaration clears the gate.
...
_Note_: a declaration covers the mutants whose replaced text contains the line it is written on, and of those the ones replacing the least. One line holds mutants at several scopes — beside `println(a < b)` sit two mutants of the comparison and one deleting the whole call — and the innermost is what somebody annotating that line is looking at, so a claim about an operator leaves the deletion still to be answered. It reaches no neighbouring statement, and a declaration covering no mutant at all is reported rather than passed over. Reaching further would let a declaration already in the tree acknowledge code a later change adds beneath it, which is excluded from the run and never counted while the change adds no line carrying the token, so nothing refers it and it merges unread.
...
**Dependency delta**:
The set difference between a manifest's base-side and head-side `{package, version}` pairs — read with `git show <merge-base>:<path>`, no worktree — classified into added packages, removed packages, and version pairs that changed. Covers `go.mod`/`go.sum`, `Cargo.lock` and `package-lock.json`; a change touching a yarn, pnpm or pip manifest is never measured, and neither is a manifest this reads fails to parse — both make the delta unmeasured, which **disqualifies** rather than exempts, the same "a gate that could not run never renders as one that passed" rule the licence gate follows.
_Note_: an added package is one whose name appears in the head-side set and not the base-side set, direct or transitive — never a version change on a name already present. It is a **Disqualifier**: #23's complaint is new untrusted code entering the tree, and a removed package is the opposite of that, so removal alone never disqualifies.
_Note_: a version pair's classification (patch, minor, major, not comparable) feeds the version-bump **Exemption**'s `versions: patch-and-minor` condition, never the disqualifier — a major bump on an already-present package is not an addition. A pre-1.0 package (`0.x`) that bumps its minor or patches within the `0.0` line classifies as not-exempt regardless of the raw semver arithmetic, because SemVer itself treats both as unstable.
_Avoid_: "dependency bump" alone for the disqualifier — a bump is a version change and never trips it; only a new name does.
...
_Note_: the grouping is forced rather than tuned. A shard is a **conflict group** — components sharing a published host port, or rooted at overlapping directories, together with everything transitively in conflict with them — so a conflicting pair is always serialised by the **Scheduler** inside one shard rather than split across two jobs that a runner may place on one host.
_Note_: a shard reports exactly the components it is **responsible** for and nothing about any other, so every declared component appears exactly once across the shards. That is what makes a dead shard detectable: completeness is a question about the declaration, asked once when the shards' documents are folded, and never about any single shard. A shard's own document is partial by construction and is not a defect. See [ADR 0026](docs/adr/0026-a-shard-reports-what-it-owns-and-the-fold-decides-completeness.md).
...
## Flagged ambiguities
**"Threshold"** for referral — there is none, deliberately (the **CRAP index**'s 30 is a **Gate**'s threshold and says nothing about referral). Because **Exemption**s are an allowlist and the default is to refer, lydite computes no riskiness score and has nothing to tune. `coverage.tolerance` and `coverage.patch.tolerance` are **Gate** knobs and say nothing about whether a change is referred.
