...
var releaseBaseURL = "https://github.com/lydite/lydite"
...
// current, so the nudge and `lydite update` only offer a real upgrade — never
...
func newUpdateCmd() *cobra.Command {
	return &cobra.Command{
		Use:           "update",
		Short:         "Update lydite to the latest release",
		SilenceUsage:  true,
		SilenceErrors: true,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if version == "dev" {
				return errors.New("this lydite was built from source — update via git, not 'lydite update'")
			}
			...
			if !updateAvailable(latest, version) {
				fmt.Printf("lydite v%s is already the latest release\n", version)
				return nil
			}
			fmt.Printf("updating lydite v%s -> v%s...\n", version, latest)
...
// selfUpdate downloads the raw lydite binary for the given version, verifies
...
func selfUpdate(ctx context.Context, ver, exe string) error {
	...
	tmp, err := os.CreateTemp(filepath.Dir(exe), ".lydite-update-*")
	if err != nil {
