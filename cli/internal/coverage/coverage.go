...
// poisoning cmd/lydite/coverage.go refuses to cache. Keeping the axis in
...
// module path, which is what lydite used to carry, can only ever resolve one
...
// cmd/lydite/coverage.go is the reason they leave this package at all,
...
// tooling is more varied across projects than a linter, so lydite reports
...
// silently dropped from coverage, matching how cmd/lydite/scan.go scopes
...
func Compute(ctx context.Context, dir string, cfg config.Config, source Source, reports ReportPaths, want PatchWanted) (map[string]float64, []Unit, PatchSources, func(), error) {
	...
	cleanup := func() {}
	if source == SourceRun {
		tmp, err := os.MkdirTemp("", "lydite-coverage-*")
		if err != nil {
			return nil, nil, PatchSources{}, func() {}, err
		}
		workDir = tmp
...
// "lydite/lydite"), needed to strip the package-qualified prefix
...
// (https://llvm.org/docs/CommandGuide/llvm-cov.html#export) lydite needs.
...
// rustReportCandidates are the conventional relative paths lydite checks for
...
// rustLCOVReportCandidates are the conventional relative paths lydite checks
...
// like cargo-audit/cargo-deny that lydite doesn't auto-install) or parsing
...
func rustCoverageOne(ctx context.Context, dir, crateDir, crateRelDir, workDir string, source Source, reportOverrides, lcovReportOverrides RustReportOverrides, solo, wantLCOV bool, idx int) (LineCount, string, bool) {
	...
	case SourceReport:
		...
		d, err := os.ReadFile(found) // #nosec G304 -- found is resolved from lydite's own candidate list or an explicit CLI flag, not user input
		if err != nil {
			return LineCount{}, "", false
		}
		data = d
...
// there's no lydite-configurable override, since this path is already the
...
func tsCoverage(ctx context.Context, dir string, exclude []string, source Source, install string) ([]Unit, bool) {
	...
	for _, pkgDir := range pkgDirs {
		...
		// measured, which is a different state from a report lydite expected
...
// packageJSON is the subset of package.json lydite needs to detect whether
...
func hasCoverageScript(pkgDir string) bool {
	data, err := os.ReadFile(filepath.Join(pkgDir, "package.json")) // #nosec G304 -- pkgDir comes from lydite's own detection walk, not user input
	if err != nil {
		return false
	}
	var pkg packageJSON
