...
// named for the decision the repo makes rather than for lydite's behavior:
...
// lydite gates on. That is accepted because it is opt-in per repo, and Semgrep
// still runs across every ecosystem either way. See docs/adr/0005.
type Linter string

const (
	// LinterESLint is the default: lydite's pinned ESLint +
