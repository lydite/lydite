...
// biomeReport mirrors the subset of `biome lint --reporter=json` lydite reads.
//
...
// reportableBiome decides whether a diagnostic is something lydite should fail
// on. It is the direct analogue of reportable() for ESLint, and exists for the
// same reason: lydite lints with its own standalone config, so anything the
// scanned project's own configuration drags in is a complaint about the setup
// we imposed rather than a defect in the code.
//
// Only the two groups lydite's bundled config enables count. In particular
// this drops:
//
...
//     Biome is full of suppressions for rules lydite deliberately doesn't run.
...
//     `"root": false` is *merged* into lydite's config by Biome (verified
...
// reportable() keeps ESLint's fatal diagnostics: a file lydite could not
...
// file with a syntax error) and `internalError/io` (a source file lydite cannot
...
func reportableBiome(category string) bool {
	...
	case strings.HasPrefix(category, "lint/"),
		...
		category == "format":
		// Rule opinions lydite never asked for: other lint groups (which a
