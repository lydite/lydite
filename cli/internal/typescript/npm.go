...
// ensureNPMToolchain installs one pinned npm-based tool into a lydite-managed
...
func ensureNPMToolchain(ctx context.Context, name, binName string, packageJSON, packageLock []byte) (string, error) {
	...
	dir := filepath.Join(cacheDir, "lydite", name+"-toolchain-"+hex.EncodeToString(sum[:])[:12])
	...
	if err := os.Rename(staging, dir); err != nil {
		// A concurrent lydite process may have won the race and created the
		...
		// behind (cache pruning, a manual delete under ~/.cache/lydite) leaves a
