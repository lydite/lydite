...
// detected TypeScript package using a toolchain lydite bundles and pins
...
// a lydite-managed cache directory (not via npx's ephemeral install) and the
...
// .lydite.yml's typescript.linter and docs/adr/0005.
type Linter string

const (
	// ESLint is the default: lydite's pinned ESLint + eslint-plugin-security.
...
// eslintFile / eslintMessage mirror the subset of `eslint --format json` lydite reads.
...
// reportable decides whether a message is something lydite should fail on.
//
// lydite lints with its OWN standalone config, deliberately independent of
...
// lintDir runs ESLint over one package and reports only lydite's own findings.
func lintDir(ctx context.Context, dir, eslintBin, configPath string) (executil.Result, error) {
	out, err := os.CreateTemp("", "lydite-eslint-*.json")
	if err != nil {
