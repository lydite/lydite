...
// under a directory tree, so lydite scan only runs the checks that apply.
package detect
...
// Ecosystem is a language toolchain lydite knows how to scan.
...
// Extensions maps each Ecosystem to the file extensions lydite considers
...
func isWorkspaceRoot(path string) bool {
	data, err := os.ReadFile(path) // #nosec G304 -- path is resolved by lydite's own detection walk, not user input
	if err != nil {
		return false
	}
