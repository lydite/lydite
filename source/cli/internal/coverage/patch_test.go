...
// TestNormalizeRelPathRelativeBaseDir guards the bug lydite actually ships
...
func TestParseGoProfile(t *testing.T) {
	...
	got, err := ParseGoProfile(GoModuleProfile{Profile: path, ModuleName: "fixture"}, dir)
	if err != nil {
...
// The multi-module case. A profile names files by module path, which says
// nothing about where the module sits on disk: `wardnet.network/go` lives
// under `sdk/wardnet-go`. Patch coverage works in --dir-relative paths (what
// ChangedLines emits), so the module path comes off and the module's own
// directory goes back on. Getting this wrong doesn't fail loudly — the keys
// simply match no changed file, and the language reports as unmeasured.
func TestParseGoProfileMapsModuleDirToDirRelativePaths(t *testing.T) {
	dir := t.TempDir()
	profile := "mode: set\n" +
		"wardnet.network/go/anomalies.go:3.13,5.2 2 1\n" +
		"wardnet.network/go/internal/rest/client.go:7.13,7.20 1 0\n"
	path := filepath.Join(dir, "cover.out")
	if err := os.WriteFile(path, []byte(profile), 0o600); err != nil {
		t.Fatal(err)
	}

	got, err := ParseGoProfile(GoModuleProfile{
		Profile:    path,
		ModuleName: "wardnet.network/go",
		RelDir:     "sdk/wardnet-go",
	}, dir)
	if err != nil {
		t.Fatalf("ParseGoProfile: %v", err)
	}
	want := LineHits{
		"sdk/wardnet-go/anomalies.go":            {3: 1, 4: 1, 5: 1},
		"sdk/wardnet-go/internal/rest/client.go": {7: 0},
	}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("ParseGoProfile = %+v, want %+v", got, want)
	}
...
// Generated code is nobody's to hand-test. wardnet's regenerated REST client
// was 983 of one PR's 1007 changed Go lines — counted, the patch gate becomes
// a measure of how much code was generated rather than how much was tested.
func TestParseGoProfileExcludesGeneratedFiles(t *testing.T) {
	dir := t.TempDir()
	src := "// Code generated by openapi-codegen. DO NOT EDIT.\n" +
		"\npackage rest\n\nfunc Gen() {\n\tprintln(1)\n}\n"
	if err := os.WriteFile(filepath.Join(dir, "rest.gen.go"), []byte(src), 0o600); err != nil {
		t.Fatal(err)
	}
	profile := "mode: set\nfixture/rest.gen.go:5.12,7.2 1 0\n"
	path := filepath.Join(dir, "cover.out")
	if err := os.WriteFile(path, []byte(profile), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if len(got) != 0 {
		t.Fatalf("ParseGoProfile = %+v, want no entries for a generated file", got)
	}
}

// The header only counts above the package clause — a file that merely talks
// about the convention (this repo's own patch.go does) is still real code.
func TestIsGeneratedGoFileIgnoresMatchesBelowPackageClause(t *testing.T) {
	dir := t.TempDir()
	src := "package rest\n\n// Code generated by openapi-codegen. DO NOT EDIT.\nfunc Hand() {}\n"
	p := filepath.Join(dir, "hand.go")
	if err := os.WriteFile(p, []byte(src), 0o600); err != nil {
		t.Fatal(err)
	}
	if isGeneratedGoFile(p) {
		t.Fatal("isGeneratedGoFile = true for a match below the package clause, want false")
	}
}
...
// repo (lydite run with --dir source), root-relative "source/..." keys match
