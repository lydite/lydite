...
--json` (stdout), `cargo-deny deny --format json check bans` (**stderr** — cargo-deny writes
nothing to stdout under `--format json`). cargo-deny's licence check is a separate invocation,
`check licenses` alone under a generated or a consumer's own config — see
[ADR 0038](../../../../../docs/adr/0038-a-licence-policy-gates-the-licences-a-change-introduces.md)
and `internal/licence/testdata/` for its own captures.
...
| `deny-transitive.ndjson` | 2 | a banned crate reached through a dependency path, from a `check bans` run (`auditprobe`) |
