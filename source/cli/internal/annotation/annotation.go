...
func gather(rest string, open int, next []Comment) (reason string, consumed int, ok bool) {
	...
	for used := range len(next) + 1 {
		...
		if used == len(next) || next[used].Line != open+used+1 {
			return "", used, false // [lydite:exclude_from_mutation][the reason beside a false
			// third value is read by nobody: Declarations answers ErrNoReason on one and never
