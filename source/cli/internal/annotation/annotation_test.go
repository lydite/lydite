...
func TestADeclarationIsReadWithItsReason(t *testing.T) {
	got, err := Declarations("a.go", []Comment{
		{Line: 4, Text: Marker + " the caller bounds n"},
...
func TestADeclarationIsKeyedToTheLineItIsWrittenOn(t *testing.T) {
	got, err := Declarations("a.go", []Comment{{Line: 4, Text: Marker + " bound is arbitrary"}})
	if err != nil {
		t.Fatalf("Declarations: %v", err)
	}
	if len(got) != 1 {
...
func TestATokenMustEndWhereItIsWritten(t *testing.T) {
	got, err := Declarations("a.go", []Comment{{Line: 1, Text: Marker + "foo"}})
	if err != nil {
		t.Fatalf("Declarations: %v", err)
	}
	if len(got) != 0 {
		t.Errorf("%q was read as a declaration: %v", Marker+"foo", got)
	}
}
...
func TestABlockCommentIsNotADeclaration(t *testing.T) {
	got, err := Declarations("a.go", []Comment{{Line: 2, Text: "/* " + Marker + " nope */"}})
	if err != nil {
		t.Fatalf("Declarations: %v", err)
	}
	if len(got) != 0 {
...
func TestABareDeclarationIsAnError(t *testing.T) {
	...
	_, err := Declarations("a.go", []Comment{{Line: 6, Text: Marker}})
	...
	if _, err := Declarations("a.go", []Comment{{Line: 6, Text: Marker + "   "}}); !errors.As(err, &want) {
