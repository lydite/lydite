...
comparison rather than relaying a caller's verdict, and it trusts a standing `lydite/clearance`
status only when that status's own `creator` login is the lydite App's bot user — see
...
in the clearance it records, so every merge-queue comparison answers `pending` until that lands
