...
`coverage.floor` in `.lydite.yml` is the second question, stated directly.
It is the minimum percentage any single measured unit must reach, it defaults
to `0` (off), and `cmd/lydite/coverage.go`'s `floorReport` prints one `[FAIL]`
line per unit below it. Opt-in, because upgrading lydite must not start failing
...
Every entry on a consumer's `lydite` branch was recorded under the mean.
...
Release notes must say this, because the step change is visible whatever lydite
does about it.
