...
upper bound — `>=4.8.4 <6.1.0` at the time of writing — so lydite's own pin
...
* **CI stayed green through it.** lydite's own repository has no TypeScript to
  scan — its only `package.json` files are the pin manifests, which
  `.lydite.yml` excludes by name — so `self-scan` never exercises the install
...
**Semgrep is**, and lydite already runs it across every ecosystem at
`config: auto`.
...
starts a window in which lydite's pin cannot advance, and closing that window
...
A `.lydite.yml` carrying `typescript.linter: eslint` is **rejected**, with an
