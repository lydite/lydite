...
lydite's Go coverage (`internal/coverage.goCoverage`) discovers every Go
...
module-scoped, and lydite ran all three at `--dir`. In a monorepo that is the
...
constraint on a repo's own toolchain and linting for lydite's convenience,
