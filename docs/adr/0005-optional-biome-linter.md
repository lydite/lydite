...
The consuming projects are migrating from ESLint to Biome, so `.lydite.yml` grows
`typescript.linter: eslint|biome` — a validated enum defaulting to `eslint`, mirroring
`coverage.source`'s precedent of naming the decision the repo makes rather than lydite's
...
Switching a repo changes what lydite gates on, in both directions. It loses ESLint's
`detect-*` heuristics, and it gains Biome's `correctness` group — lydite's TypeScript check
...
opt-in is the one outcome nobody can detect: the repo believes it has migrated, and lydite
