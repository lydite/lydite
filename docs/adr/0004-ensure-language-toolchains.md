# lydite ensures the language toolchain each detected ecosystem needs
...
command runs. It is called from both `cmd/lydite/scan.go` and
`cmd/lydite/coverage.go`, for the enabled ecosystems `internal/detect` found.

This completes the "pin the exact toolchain, don't reuse ambient installs"
principle stated in `internal/golang`. lydite already provisions everything
...
## Versions come from the repo's manifests, not from `.lydite.yml`
...
own tooling. A version restated in `.lydite.yml` could only agree redundantly
...
runners lydite actually runs on that is the common case — so the common path
...
Only one of the three involves lydite downloading anything.
...
- **Go** delegates to `GOTOOLCHAIN`. Any Go 1.21+ fetches another toolchain
  ...
  with no Go, or one older than 1.21, does lydite download a tarball from
...
- **Rust** delegates to rustup, which already reads the same
  ...
  With no rustup present lydite reports that and continues; installing rustup
  itself would be a larger, less reversible thing to do unasked.
- **Node** is downloaded and unpacked by lydite, from nodejs.org, with the
...
when lydite installs nothing. Its default (`auto`) does not only *upgrade*:
...
same place, or lydite concludes "ambient already satisfies" and then lands on
...
Consumers no longer need to pin `go-version` on lydite's account.
...
- `config.Config` gains a `Toolchain` section; `internal/toolchain` does not
  import `internal/config`, so config stays a leaf. `cmd/lydite/toolchain.go`
  maps between them.
- The resolved environment is applied by mutating this process's `PATH` and
  ...
  `Compute` signature. lydite is a single-shot CLI; this is what a shell
...
- `cmd/lydite/coverage.go` now detects ecosystems *before* calling
...
- Downloads land in the existing version-keyed `~/.cache/lydite` layout, so a
...
- rustup selection is distinct from rustup installation. A channel read from a
  ...
  in; a channel supplied by `toolchain.rust` in `.lydite.yml` is invisible to
  ...
  different channels.
- An override that lydite cannot parse as a version is a hard config error
...
- Only enabled ecosystems are provisioned. A language disabled in
  `.lydite.yml` is one whose tools never run, and on a repo that disabled
