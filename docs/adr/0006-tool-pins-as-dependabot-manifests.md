...
Every tool lydite installs — ESLint, Biome, cargo-audit, cargo-deny, gosec, govulncheck,
...
age them out. Pinning the toolchain is lydite's whole premise; a pinned *security* toolchain
...
show up in lydite's actual use of them.
...
directives in lydite's own `go.mod`. The latter works and is more idiomatic, but it drags
...
Dependabot at dozens of transitive dependencies lydite does not use.
